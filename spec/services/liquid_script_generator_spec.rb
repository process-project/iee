...
describe LiquidScriptGenerator do
  it 'inserts active grant id' do
    script = described_class.new(build(:computation), '{{ grant_id }}').call
  ...
  context 'when generating curls' do
    ...
    it 'inserts upload file curl for file path' do
      script = described_class.new(
        computation,
        '<%= path/to/foo.gif | stage_in_path: "out.gif" %>'
      ).call
    ...
    it 'add --fail flag when download is not optional' do
      ...
      script = described_class.new(computation,
                                   '<%= dir/foo.txt | stage_in_path %>').call
    ...
    it 'inserts case number of the patient' do
      script = described_class.new(computation, '<%= patient.case_number %>').call
  ...
  it 'inserts gitlab ssh download key payload' do
    script = described_class.new(create(:rimrock_computation),
  ...
  it 'inserts repository sha to clone' do
    script = described_class.new(create(:rimrock_computation, revision: 'rev'),
                                 '<%= revision %>').call
  ...
  it 'inserts pipeline identifier' do
    ...
    script = described_class.new(computation, '<%= pipeline_identifier %>').call
  ...
  it 'inserts patient case_number' do
    ...
    script = described_class.new(computation, '<%= case_number %>').call
